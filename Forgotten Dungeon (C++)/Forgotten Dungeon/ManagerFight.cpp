...
ManagerFight::ManagerFight() : Manager()
{
	hero = nullptr;
	enemy = nullptr;
...
void ManagerFight::Init(Hero *hero_, Enemy *enemy_)
{
	hero = hero_;
	enemy = enemy_;
	fightLog->clear();
...
void ManagerFight::EndFight()
{
	while (!fightLog->empty())
		fightLog->pop_front();
}

void ManagerFight::HitRatio()
{
...
void ManagerFight::CompareIndicator(double HRIndicator, double *calculatedHR)
{
	if (HRIndicator < 0)
	{
...
void ManagerFight::SetUsedSkill(int index) { usedSkill = index; }
bool ManagerFight::GetFightStatus() { return fightFinished; }
bool ManagerFight::CheckIfHeroEscaped() { return escaped; }
void ManagerFight::ResetHeroEscaped() { escaped = false; }

void ManagerFight::DecrementCooldowns()
{
	for (int i = 0; i < 6; i++)
...
void ManagerFight::Draw(Resources *resources, ALLEGRO_Font *allegro_font)
{
